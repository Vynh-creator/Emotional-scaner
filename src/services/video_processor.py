...
from src.utils.helpers import setup_logging, format_results
...
w2v_model, processor, models_out, detector = None, None, None, None
...
class VideoRecorder(QWidget):
    ...
    def init_camera(self):
        ...
        if not self.cap.isOpened():
            ...
            self.preview_label.setText("📷 Камера готова")
    def init_ui(self):
        ...
        if status and status.input_overflow:
            with self.buffer_lock:
                if len(self.audio_buffer) > 20:
                    self.audio_buffer = self.audio_buffer[-20:]
        elif status:
            pass
        if self.is_recording:
            with self.buffer_lock:
    ...
    def on_chunk_tick(self):
        ...
        live_v = live_v[-int(max(1, interval_s * 15)):]
        live_a = live_a[-int(max(1, interval_s * 25)):]
